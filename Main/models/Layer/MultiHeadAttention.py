...
import numpy as np
from Main.models.Encoder import PositionEncoder
from Main.models.Layer import Attention
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, config, d_model, n_head, attention_mask=None):
        ...
        self.proj_layer_query = nn.ModuleList(
            [nn.Linear(self.config.d_model, self.d_v) for _ in range(self.config.n_head)])
    ...
    def forward(self, query, key, val, key_structure=None, val_structure=None, attention_mask=None):
        ...
        query_head = torch.zeros((self.n_head, *query.shape[:-1], self.d_k), device=query.device)
        key_head = torch.zeros((self.n_head, *query.shape[:-1], self.d_k), device=query.device)
        val_head = torch.zeros((self.n_head, *query.shape[:-1], self.d_k), device=query.device)
        ...
        if key_structure is not None and val_structure is not None:
            ...
            self_atten_features, atten_values = self.attention(query_head, key_head, val_head,
                                                               key_structure=key_structure, val_structure=val_structure,
