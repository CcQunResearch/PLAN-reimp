...
from Main.models.Encoder import LearnedPositionEncoder
...
class PostModule(nn.Module):
    ...
    def __init__(self, config):
        ...
        # <----------- Final layer to predict the output class (1 classes) (To map emb to classes) ----------->
        self.final_layer_emb = nn.Sequential(nn.Linear(self.config.emb_dim, 1),
                                             nn.Sigmoid())
    ...
    def forward(self, X_word, time_delay, batch_size, num_posts, emb_dim, structure=None, attention_mask=None):
        ...
        if self.config.post_module_version == 3:
            ...
            # <----------- Getting the predictions ----------->
            output = self.final_layer_emb(self_atten_output_post).view(-1)
