...
class WordModule(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.word_module_version in [2, 3, 4]:
            ...
            self.transformer_word = Transformer.Transformer(self.config, self.config.n_mha_layers_word,
    ...
    def forward(self, X, word_pos, attention_mask=None):
        ...
        if self.config.word_module_version == 4:
            ...
            words_attention_weights = F.softmax(
                words_attention_values.permute(0, 1, 3, 2) + attention_mask.unsqueeze(-2), dim=-1)
